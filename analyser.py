...
def analyse_headers(headers):
    ...
    if from_addr and reply_to and from_addr.strip() != reply_to.strip():
        score += 30
        issues.append("Incohérence entre De et Répondre à")

    if from_domain in BLACKLIST_DOMAINS:
        score += 50
        issues.append(f"Domaine connu pour phishing : {from_domain}")
    elif from_domain and from_domain not in WHITELIST_DOMAINS:
        if any(legit in from_domain for legit in ["paypal", "amazon", "banque", "gmail"]):
            score += 40
...
def analyse_liens(body, sender_domain):
    ...
    for link in links:
        ...
        if display_text.lower() not in url_domain and sender_domain not in url_domain:
            score += 25
